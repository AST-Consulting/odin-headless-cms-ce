...
import { ArrowLeft, Save, Code, FileVideo, Plus, X, User, ExternalLink, Info, Loader2, Monitor, ImageIcon, Play, RotateCcw } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import { toast } from "sonner";
import { getWebStoryTemplateById, updateWebStoryTemplate } from "@/lib/api";
...
export default function EditTemplatePage() {
    ...
    useEffect(() => {
        if (hasHydrated) {
            ...
            if (!havePermission(user, "webstory-template", "edit")) {
    ...
    const fetchTemplate = useCallback(async () => {
        setLoading(true);
        try {
            const data = await getWebStoryTemplateById(id);
            setName(data.name);
            setSlug(data.slug || "");
            ...
            // Handle default author for legacy documents or empty author list
            if ((!data.authors || data.authors.length === 0) && user) {
                setAuthors([{
                    id: user.id,
                    name: user.name,
                    slug: user.slug || ""
    ...
    const handleUpdate = async () => {
        ...
        setSaving(true);
        try {
            await updateWebStoryTemplate(id, {
                name,
                slug,
                authors,
                html,
    ...
    if (loading) {
        return (
            <div className="h-[60vh] flex flex-col items-center justify-center gap-4">
                ...
                <p className="font-medium text-muted-foreground">Hydrating layout architect...</p>
            </div>
        );
    }
        ...
        <div className="max-w-6xl mx-auto space-y-6 pb-20">
            <div className="flex items-center justify-between sticky top-0 bg-background/80 backdrop-blur-md z-10 py-4 border-b">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight">Edit Story Template</h1>
                        <p className="text-sm text-muted-foreground mt-0.5">Refine your AMP structures and dynamic variables.</p>
                    </div>
                </div>
                <div className="flex items-center gap-3">
                    <Button variant="outline" onClick={() => router.back()} className="h-11 px-6 rounded-xl">Cancel</Button>
                    <Button onClick={handleUpdate} disabled={saving} className="gap-2 h-11 px-6 rounded-xl shadow-lg shadow-primary/20">
                        ...
                        <CardContent className="pt-6 space-y-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {allowedFields.map(field => (
                                    <div key={field} className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-primary/10 border border-primary/20 transition-all group">
                                ...
                                {allowedFields.length === 0 && (
                                    <p className="text-xs text-muted-foreground pt-2 text-center w-full italic">No dynamic fields registered yet.</p>
                                )}
                            </div>
                        </CardContent>
                    </Card>
