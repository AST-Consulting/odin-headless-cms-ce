...
import { getPageById, updatePage, type Page } from "@/lib/api";
...
export default function EditStaticPage() {
    ...
    useEffect(() => {
        if (pageId && selectedProperty) {
            loadPage();
        }
    }, [pageId, selectedProperty]);
    ...
    const loadPage = async () => {
        ...
        try {
            ...
            setSelectedTags(page.tags?.map((t) => (typeof t === "string" ? t : t.id)) || []);
            setSelectedCategories(page.categories?.map((c) => (typeof c === "string" ? c : c.id)) || []);

            if (page.seo) {
                setSeoTitle(page.seo.title || "");
                setSeoDescription(page.seo.metaDescription || "");
                setSeoKeywords(page.seo.keywords?.join(", ") || "");

                const seo = page.seo as any;
                if (seo.og) {
                    setOgTitle(seo.og.title || "");
                    setOgDescription(seo.og.description || "");
                    setOgUrl(seo.og.url || "");
                    setOgImage(seo.og.image || "");
                }
            }
        } catch (error: any) {
            toast({
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Wait for Elasticsearch to index the updated page before navigating
    ...
    if (initialLoading) {
        return (
            <div className="w-full max-w-full py-4 sm:py-6">
                <div className="flex items-center justify-center h-64">
                    <p className="text-sm sm:text-base text-muted-foreground">Loading page...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="w-full max-w-full space-y-4 sm:space-y-6">
            ...
            <div className="flex items-center gap-2 sm:gap-4">
                ...
                <div className="min-w-0">
                    <h1 className="text-xl sm:text-2xl md:text-3xl font-bold truncate">Edit Static Page</h1>
                    <p className="text-sm sm:text-base text-muted-foreground mt-0.5 sm:mt-1 truncate">
                        Update your static page content and settings
                    </p>
                </div>
            </div>
