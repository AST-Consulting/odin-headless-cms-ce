...
import { History, Pencil, Trash2, ChevronDown, ChevronUp, Link as LinkIcon, Hash, Tag as TagIcon } from "lucide-react";
...
interface TagCardProps {
    ...
    onStatusChange: (tag: Tag, checked: boolean) => Promise<void>;
            ...
            <Card className="bg-white dark:bg-slate-900/50 rounded-[22px] shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden">
                <CardContent className="p-5">
                    <div className="flex items-start justify-between gap-3 mb-4">
                        <div className="flex-1 min-w-0 text-left">
                            ...
                            <div className="flex items-center gap-2 mt-1.5 text-slate-500 dark:text-slate-400">
                                ...
                                <span className="text-[13px] font-medium truncate">{tag.slug || tag.name}</span>
                            </div>
                        </div>
                        <div className="flex flex-col items-end gap-2 shrink-0">
                            <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-800 px-2 py-1 rounded-lg border border-slate-100 dark:border-slate-700">
                                ...
                                <span className="text-[12px] font-bold text-slate-700 dark:text-slate-300">{tag.rank}</span>
                            </div>
                            <Switch
                                checked={tag.status === 'active'}
                                onCheckedChange={(checked) => onStatusChange(tag, checked)}
                                className="scale-90"
                            />
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        ...
                        <Badge variant="outline" className="text-[10px] bg-slate-50 text-slate-600 border-slate-100 uppercase tracking-wider font-bold">TAG</Badge>
                    </div>

                    {tag.description && (
                        <p className="mt-3 text-[13px] text-slate-600 dark:text-slate-400 line-clamp-2 text-left">
                            {tag.description}
                        </p>
                    )}
                </CardContent>
                ...
                <CollapsibleContent>
                    <div className="grid grid-cols-2 gap-y-4 gap-x-4 px-5 py-4 bg-[#f8fafc] dark:bg-slate-800/50 border-t border-gray-50 dark:border-slate-800">
                        {[
                            { label: "Created By", value: tag.createdBy?.name || "-" },
                            { label: "Role", value: tag.createdBy?.userType || "-" },
                            { label: "Created At", value: formatDate(tag.createdAt) },
                            { label: "Updated At", value: formatDate(tag.updatedAt) },
                        ].map(({ label, value }) => (
