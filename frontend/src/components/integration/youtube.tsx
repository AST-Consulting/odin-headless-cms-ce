...
import { Loader2, Eye, Users, Clock, Settings, Youtube, TrendingUp } from "lucide-react";

import type { IntegrationRecord, YouTubeAccount, YouTubeData } from "@/lib/types";
import {
...
export function YouTubeCard({
    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            setError(null);
            try {
                const data = await fetchYouTubeAccounts(propertyId);
    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            try {
                const data = await fetchYouTubeData(propertyId);
                if (!cancelled) setYouTubeData(data);
            } catch (err: any) {
                if (!cancelled) setError(err.message);
            } finally {
    ...
    const handleDisconnect = async () => {
        ...
        setError(null);
        try {
            await disconnectIntegration(propertyId, "youtube");
            onRefresh();
            setIsSettingsOpen(false);
        } catch (err: any) {
            setError(err.message);
        ...
        <>
            <Card className={`relative flex flex-col h-full transition-shadow ${borderColor}`}>
                ...
                <CardContent className="p-5 flex flex-col flex-1 mt-1">
                    <div className="w-12 h-12 rounded-xl mb-4 flex items-center justify-center p-1 border shadow-sm bg-white">
                        <Youtube className="w-full h-full text-red-600 p-1" />
                    </div>

                    <h3 className="font-semibold text-lg mb-1">YouTube Analytics</h3>
            ...
            <Dialog open={isSettingsOpen} onOpenChange={setIsSettingsOpen}>
                <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                        <DialogTitle>YouTube Analytics Configuration</DialogTitle>
                        <DialogDescription>
                            {isPending
                                ? "Select a YouTube channel to sync your metrics."
                                : "View your connection details and channel performance."}
                        </DialogDescription>
                    </DialogHeader>
                            ...
                            <div className="space-y-5">
                                {integration?.metadata?.label && (
                                    <div className="bg-muted/50 p-3 rounded-md border">
                                        <p className="text-xs text-muted-foreground mb-1">Connected Channel</p>
                                ...
                                {isLoadingData ? (
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="bg-muted/30 rounded-lg p-3 border space-y-1">
                                            ...
                                            <p className="text-xl font-semibold">{youtubeData.views.toLocaleString()}</p>
                                        </div>
                                        <div className="bg-muted/30 rounded-lg p-3 border space-y-1">
                                            <div className="flex items-center gap-2 text-muted-foreground">
                                                <Clock className="w-4 h-4" />
                                                <span className="text-xs font-medium">Watch Time (m)</span>
                                            </div>
                                            <p className="text-xl font-semibold">{youtubeData.estimatedMinutesWatched.toLocaleString()}</p>
                                        </div>
                                        <div className="bg-muted/30 rounded-lg p-3 border space-y-1">
                                            <div className="flex items-center gap-2 text-muted-foreground">
                                                <TrendingUp className="w-4 h-4" />
                                                <span className="text-xs font-medium">New Subs</span>
                                            </div>
                                            <p className="text-xl font-semibold">{youtubeData.netSubscribersGained > 0 ? "+" : ""}{youtubeData.netSubscribersGained.toLocaleString()}</p>
                                        </div>
                                    </div>
                                ) : null}
                            </div>
