...
import { Loader2, Eye, Users, Settings, Instagram, MousePointerClick } from "lucide-react";

import type { IntegrationRecord, InstagramAccount, InstagramData } from "@/lib/types";
import {
...
export function InstagramCard({
    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            setError(null);
            try {
                const data = await fetchInstagramAccounts(propertyId);
    ...
    // Fetch Instagram data when connected
    useEffect(() => {
        ...
        async function load() {
            ...
            try {
                const data = await fetchInstagramData(propertyId);
                if (!cancelled) setInstagramData(data);
            } catch (err: any) {
                if (!cancelled) setError(err.message);
            } finally {
    ...
    const handleDisconnect = async () => {
        ...
        setError(null);
        try {
            await disconnectIntegration(propertyId, "instagram");
            onRefresh();
            setIsSettingsOpen(false);
        } catch (err: any) {
            setError(err.message);
        ...
        <>
            <Card className={`relative flex flex-col h-full transition-shadow ${borderColor}`}>
                ...
                <CardContent className="p-5 flex flex-col flex-1 mt-1">
                    <div className="w-12 h-12 rounded-xl mb-4 flex items-center justify-center p-1 border shadow-sm bg-white">
                        <Instagram className="w-full h-full text-[#E1306C] p-1" />
                    </div>

                    <h3 className="font-semibold text-lg mb-1">Instagram Insights</h3>
                    <p className="text-sm text-muted-foreground leading-snug flex-1 mb-6">
                        Track your Instagram Account reach and impressions.
                    </p>
            ...
            <Dialog open={isSettingsOpen} onOpenChange={setIsSettingsOpen}>
                <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                        <DialogTitle>Instagram Insights Configuration</DialogTitle>
                        <DialogDescription>
                            {isPending
                                ? "Select an Instagram Account to sync your metrics."
                                : "View your connection details and account performance."}
                        </DialogDescription>
                    </DialogHeader>
                                ...
                                {isLoadingData ? (
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="bg-muted/30 rounded-lg p-3 border space-y-1">
                                            ...
                                            <p className="text-xl font-semibold">{instagramData.reach.toLocaleString()}</p>
                                        </div>
                                        <div className="bg-muted/30 rounded-lg p-3 border space-y-1">
                                            <div className="flex items-center gap-2 text-muted-foreground">
                                            ...
                                            <p className="text-xl font-semibold">{instagramData.impressions.toLocaleString()}</p>
                                        </div>
                                    </div>
                                ) : null}
                            </div>
