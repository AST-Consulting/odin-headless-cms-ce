...
import { Loader2, MousePointerClick, Eye, BarChart3, MapPin, Settings } from "lucide-react";

import type { IntegrationRecord, GSCAccount, GSCData } from "@/lib/types";
import {
...
export function GoogleSearchConsoleCard({
    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            setError(null);
            try {
                const data = await fetchGSCAccounts(propertyId);
    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            try {
                const data = await fetchGSCData(propertyId);
                if (!cancelled) setGscData(data);
            } catch (err: any) {
                if (!cancelled) setError(err.message);
            } finally {
    ...
    const handleDisconnect = async () => {
        ...
        setError(null);
        try {
            await disconnectIntegration(propertyId, "search_console");
            onRefresh();
            setIsSettingsOpen(false);
        } catch (err: any) {
            setError(err.message);
        ...
        <>
            <Card className={`relative flex flex-col h-full transition-shadow ${borderColor}`}>
                ...
                <CardContent className="p-5 flex flex-col flex-1 mt-1">
                    ...
                    <p className="text-sm text-muted-foreground leading-snug flex-1 mb-6">
                        Track your site&apos;s search performance and identify SEO opportunities.
                    </p>
            ...
            <Dialog open={isSettingsOpen} onOpenChange={setIsSettingsOpen}>
                <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                        <DialogTitle>Google Search Console Configuration</DialogTitle>
                        <DialogDescription>
                            {isPending
                                ? "Select a Search Console site to sync your metrics."
                                : "View your connection details and search performance."}
                        </DialogDescription>
                    </DialogHeader>
                            ...
                            <div className="space-y-5">
                                {integration?.metadata?.label && (
                                    <div className="bg-muted/50 p-3 rounded-md border">
                                        <p className="text-xs text-muted-foreground mb-1">Connected Site</p>
                                ...
                                {isLoadingData ? (
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="bg-muted/30 rounded-lg p-3 border space-y-1">
                                            ...
                                            <p className="text-xl font-semibold">{gscData.clicks.toLocaleString()}</p>
                                        </div>
                                        <div className="bg-muted/30 rounded-lg p-3 border space-y-1">
                                            <div className="flex items-center gap-2 text-muted-foreground">
                                            ...
                                            <p className="text-xl font-semibold">{gscData.impressions.toLocaleString()}</p>
                                        </div>
                                        <div className="bg-muted/30 rounded-lg p-3 border space-y-1">
                                            <div className="flex items-center gap-2 text-muted-foreground">
                                            ...
                                            <p className="text-xl font-semibold">{gscData.ctr.toFixed(2)}%</p>
                                        </div>
                                        <div className="bg-muted/30 rounded-lg p-3 border space-y-1">
                                            <div className="flex items-center gap-2 text-muted-foreground">
                                            ...
                                            <p className="text-xl font-semibold">{gscData.position.toFixed(1)}</p>
                                        </div>
                                    </div>
                                ) : null}
                            </div>
